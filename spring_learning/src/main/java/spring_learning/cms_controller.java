...
public class cms_controller {
  ...
  public String cmsview(Model m) {
	  //데이터 그룹 한개만 가져옴 
	  /*
	   mapper에서 resultType String으로 처리했을 경우 하나의 컬럼 값에 대해서만 
	   처리할때 사용할 수 있습니다. 단,여러개의 컬럼으로 처리시에는 첫번째 컬럼 외에 모두 loss로 처리합니다.
	   (예시: count, avg, sum, max, min) 
	   */
  ...
  public String cmsok(@RequestParam String csubject,
    @RequestParam String cuser,
	  ...
	  //checkbox 가 동일한 이름에 여러개가 잇을 경우 배열로 값을 받으며,DB 저장 시
	  //String 으로 변환하여 String.join()클래스 이용하여 DB set 이라는 자료형으로 저장함
